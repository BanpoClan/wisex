...
wisex = {};
//返回上一页
wisex.goback = function(e) {
    $(e).button("loading").delay(1000).queue(function() {
        history.go(-1);
    });
}
//跳转到指定的页面
wisex.goAction = function (e, moudleId, menuId, btnId, url, grid) {
    var addReg = /add/gi;
    var editReg = /edit/gi;
    ...
    debugger;
    if (addReg.test(url)) {
        wisex.addModel(e, moudleId, menuId, btnId, url, grid);
    } else if (editReg.test(url)) {
        wisex.editModel(e, moudleId, menuId, btnId, url, grid);
    } else if (deleteReg.test(url)) {
        wisex.delModel(e, moudleId, menuId, btnId, url, grid);
    } else if (authen.test(url)) {
        wisex.authen(e, moudleId, menuId, btnId, url, grid);
    }
}
//addModel
wisex.addModel = function (e, moudleId, menuId, btnId, url, grid) {
    $(e).button("loading");
    window.location.href = [url, moudleId, menuId, btnId].join("/");
}

//editModel
wisex.editModel = function (e, moudleId, menuId, btnId, url, grid) {
...
//delete
wisex.delModel = function (e, moudleId, menuId, btnId, url, grid) {
...
//authen
wisex.authen = function (e, moudleId, menuId, btnId, url, grid) {
    var editRow = grid.getSelectedRow();
    if (editRow && editRow.Id) {
        $(e).button("loading");
        window.location.href = [url, moudleId, menuId, btnId, editRow.Id].join("/");
...
wisex.buttonLoading = function(form) {
